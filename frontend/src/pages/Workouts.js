...
import { Dumbbell, Plus, Trash2, Play, Check, X, Timer, Flame, TrendingUp, Calendar, FileText, Activity } from "lucide-react";
...
export default function Workouts() {
  ...
  const handleLogWorkout = async () => {
    ...
    try {
      ...
      setNewWorkout({ activity_type: "weightlifting", name: "", duration_minutes: 30, notes: "", date: today });
      setOpenLog(false);
      // Reload data
                    ...
                    <div className="border-t border-[#27272A] pt-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <Input type="number" value={newExercise.sets} onChange={(e) => setNewExercise({...newExercise, sets: parseInt(e.target.value) || 0})} placeholder="Séries" className="bg-[#121212] border-[#27272A] text-white" />
                          <Input type="number" value={newExercise.reps} onChange={(e) => setNewExercise({...newExercise, reps: parseInt(e.target.value) || 0})} placeholder="Reps" className="bg-[#121212] border-[#27272A] text-white" />
                          <Input value={newExercise.weight} onChange={(e) => setNewExercise({...newExercise, weight: e.target.value})} placeholder="Carga" className="bg-[#121212] border-[#27272A] text-white" />
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-[#A1A1AA]">{getActivityLabel(workout.activity_type)} - {workout.duration_minutes} min</p>
                          </div>
                        </div>
                  ...
                  plans.map(plan => (
                    <Card key={plan.plan_id} className="bg-[#0A0A0A] border-[#27272A] p-4">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleDeletePlan(plan.plan_id)} className="text-red-500">
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-[#A1A1AA]">{workout.date} - {workout.duration_minutes} min</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[#00F0FF] font-mono text-sm">{workout.completed ? '+' : ''}{workout.xp_earned} XP</span>
                          <Button variant="ghost" size="sm" onClick={() => handleToggleWorkout(workout.log_id)}>
                            {workout.completed ? <Check className="w-4 h-4 text-green-500" /> : <X className="w-4 h-4 text-gray-500" />}
                          </Button>
                        </div>
                      </div>
                    </Card>
